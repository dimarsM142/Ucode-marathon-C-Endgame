...
int start_menu (SDL_Window * window, SDL_Renderer *renderer) {
    ...
    SDL_Surface *image = 
        SDL_LoadBMP("resource/img/setting.bmp");
    ...
    tempSurface = SDL_LoadBMP("resource/img/start.bmp");
    SDL_Texture *start = 
        SDL_CreateTextureFromSurface(renderer, tempSurface);
    SDL_FreeSurface(tempSurface);
    ...
    while (!quit){
        ...
        SDL_Rect start_arr = {750, 450, 350, 80};
        ...
        SDL_RenderCopy(renderer, start, NULL, &start_arr);
